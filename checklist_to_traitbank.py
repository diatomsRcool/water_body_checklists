...
#This file holds the summary statistics for each water body. The first list removes any old
#version before starting a new version
#the file paths will be to your local machine
os.remove('/Volumes/PCCOMP/waterbody_checklist_results/water_stats.txt')
out_file_x = open('/Volumes/PCCOMP/waterbody_checklist_results/water_stats.txt', 'a')
out_file_x.write('water_body	mrgid	taxa	obs\n')

#making a list of water bodies and their mrgids to iterate over
water_list = open('list_results.tsv', 'r') #list of water bodies and their mrgid
waters = []
...
#going through list of water bodies
for y in waters:
	water = y[0]
	print(water)
	mrgid = y[1]
	...
	in_file = open('/Volumes/PCCOMP/waterbody_checklist_results/' + water + '/' + water + '.tsv', 'r')
	shutil.copy('meta.xml','/Volumes/PCCOMP/waterbody_checklist_results/' + water + '/')
	shutil.copy('tb_references.txt','/Volumes/PCCOMP/waterbody_checklist_results/' + water + '/')
	counter = 100000
	kingdoms = []
	phyla = []
	classes = []
	...
	for line in in_file:
		...
		if species_e == '' or genus == '': #if its not identified to species, then we don't want it
			...
			if taxon in species: 
				...
				species.append(taxon) #giving each taxon a local identifier from here to line 124
			...
			if kingdom in kingdoms or kingdom == '':
				...
				counter = counter + 1
			parent_dict[taxon] = taxon_id[genus] #because we ignored everything that was 
			#missing a specific epithet or a genus, we don't have to worry about either of them missing
	...
	out_file = open('/Volumes/PCCOMP/waterbody_checklist_results/' + water + '/tb_measurement.txt', 'w')
	out_file_t = open('/Volumes/PCCOMP/waterbody_checklist_results/' + water + '/tb_taxon.txt', 'w')
	out_file_c = open('/Volumes/PCCOMP/waterbody_checklist_results/' + water + '/tb_occurrence.txt', 'w')
	...
	record_total = 0 #keeping track of the total number of records for each water body
	in_file.seek(0)
	for line in in_file:
		line = line.strip('\n')
		row = line.split('\t')
		...
		if species == '' or genus == '': #if its not identified to species, then we don't want it
			...
			if taxon in taxa: #remove accidental duplication
				...
				polygon = polygons[mrgid] #look up polygon for the water body
				...
				out_file.write(meas_id + '\t' + occur_id + '\t' + '' + '\t' + 'true' + '\t' + 'http://eol.org/schema/terms/Present' + '\t' + 'http://marineregions.org/mrgid/' + mrgid + '\t' + 'R01|R02' + '\t' + 'Compiler: Anne E Thessen' + '\t' + 'http://gimmefreshdata.github.io/?limit=5000000&taxonSelector=' + taxon + '&traitSelector=&wktString=' + polygon + '\n')
	...
	out_file_x.write(water + '\t' + mrgid + '\t' + str(len(taxa)) + '\t' + str(record_total) + '\n') #output stats
	print('TB files complete') #make sure code gets to end
	#closing the input and output files for one water body before moving on to the next
	in_file.close()
	out_file.close()
	out_file_t.close()
	out_file_c.close()
